...
export function OAuthButtons({ callbackUrl = '/', enabledProviders }: Props) {
  const [loading, setLoading] = useState<OAuthProviderId | null>(null);

  function handleSignIn(provider: OAuthProviderId) {
    setLoading(provider);
  ...
  if (enabledProviders.length === 0) {
    return (
      <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-900">
        <p className="font-medium">Đăng nhập chưa sẵn sàng</p>
        <p className="mt-1 text-amber-800">
          Chưa cấu hình OAuth trên server. Thêm credentials Google, GitHub hoặc Facebook vào env.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {enabledProviders.map((provider) => {
        const { label, className, icon } = PROVIDER_BUTTONS[provider];
        return (
          <button
            key={provider}
            type="button"
